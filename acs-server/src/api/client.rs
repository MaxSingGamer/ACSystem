...
//!
//! 认证：全部使用镜像 apikey（`mirror_keys` 表，Active），与只读镜像同一凭证体系。
...
/// 校验镜像 apikey 有效（Active）。
fn check_apikey(conn: &Connection, apikey: &str) -> bool {
    conn.query_row(
...
async fn open_account(
    ...
    if !check_apikey(&conn, &req.apikey) {
